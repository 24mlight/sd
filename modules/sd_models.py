...
class CheckpointInfo:
    ...
    def register(self):
        checkpoints_list[self.title] = self # CheckpointInfo
        for id in self.ids:
            checkpoint_alisases[id] = self # CheckpointInfo
...
def select_checkpoint():
    ...
    checkpoint_info = checkpoint_alisases.get(model_checkpoint, None) # 获取值，值类型为
    if checkpoint_info is not None:
        return checkpoint_info
...
def load_model(checkpoint_info=None, already_loaded_state_dict=None, time_taken_to_load_state_dict=None):
    ...
    sd_config = OmegaConf.load(checkpoint_config) # checkpoint_config其实就是.yaml文件
    ...
    sd_model.used_config = checkpoint_config # yaml
...
def reload_model_weights(sd_model=None, info=None):
    ...
    checkpoint_info = info or select_checkpoint() # CheckpointInfo 
